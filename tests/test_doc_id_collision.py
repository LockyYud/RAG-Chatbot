...
from ragbench.datasets.synthetic import chunk_corpus
...
def test_synthetic_dataset_chunking_disambiguates_same_named_files(tmp_path: Path) -> None:
    """Synthetic generation now chunks through the pipeline chunker (v2), so the
    doc_id disambiguation this pins is the parser's, shared with every ingest."""
