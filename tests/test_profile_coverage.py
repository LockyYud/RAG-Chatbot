...
def test_declared_retrieval_coverage_is_enforced_for_rag_profiles_too() -> None:
    """Claim-eligible RAG suites are required to declare
    min_retrieval_coverage (see suites._REQUIRED_COVERAGE_KEYS) because their
    reports publish recall/nDCG/MRR. That requirement is only meaningful if
    the declared threshold actually constrains a non-retrieval profile — it
    used to be read for the retrieval profile alone."""
    items = [_item("q1", expected_doc_ids=["d1"])] + [_item(f"q{i}") for i in range(2, 21)]  # 1/20 = 5%
    with pytest.raises(ValueError, match="single_hop_rag profile requires >= 95%"):
        validate_profile("single_hop_rag", items, coverage={"min_retrieval_coverage": 0.95})


def test_undeclared_retrieval_coverage_does_not_impose_a_qrel_floor_on_rag_profiles() -> None:
    """A RAG dataset with no qrels at all is a legitimate answer-quality-only
    evaluation. Enforcing a declared ratio for every profile must not drag the
    *undeclared* "at least one qrel" fallback along with it."""
    validate_profile("single_hop_rag", [_item("q1"), _item("q2")])  # does not raise


def test_declared_zero_retrieval_coverage_accepts_a_dataset_without_qrels() -> None:
    validate_profile("single_hop_rag", [_item("q1"), _item("q2")], coverage={"min_retrieval_coverage": 0.0})
