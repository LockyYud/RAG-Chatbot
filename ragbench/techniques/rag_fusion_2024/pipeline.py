...
from ragbench.core.text import dense_cosine, token_count
...
class RAGFusionRetriever:
    ...
    def retrieve(self, query: str, top_k: int) -> tuple[list[RetrievalResult], dict[str, Any]]:
        ...
        (``ragbench eval/bench --concurrency``) could interleave. A
        ...
        for expanded_query in all_queries:
            ...
            ranked = sorted(
                ((node, dense_cosine(qv, node.embedding if node.embedding is not None else [])) for node in self.nodes),
