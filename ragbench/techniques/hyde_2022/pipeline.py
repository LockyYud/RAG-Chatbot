...
from ragbench.core.schema import ArtifactManifest, IndexedNode, RAGAnswer, RetrievalResult
from ragbench.core.text import dense_cosine, mean_dense_vector, token_count
from ragbench.indexing.artifacts import default_store_backend, load_vector_store, save_nodes
...
from ragbench.processing.chunkers.recursive import RecursiveChunker
...
class HyDERetriever:
    ...
    def retrieve(self, query: str, top_k: int) -> tuple[list[RetrievalResult], dict[str, Any]]:
        ...
        pipeline (``ragbench eval/bench --concurrency``) could interleave. A
        ...
        scored = [
            (node, dense_cosine(proxy_vec, node.embedding if node.embedding is not None else []))
...
class HyDEPipeline(BasePipeline):
    ...
    def load(self, artifact_path: str) -> None:
        ...
        # concurrent query() calls (ragbench eval/bench --concurrency).
