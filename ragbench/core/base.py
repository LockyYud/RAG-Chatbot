...
* :meth:`query` runs the technique's inference flow against the state built
  ...
  follows the schema produced by :func:`ragbench.core.measure.build_query_metadata`.
...
from ragbench.core.schema import JSONValue
from ragbench.core.spec import TechniqueSpec, technique_spec

if TYPE_CHECKING:
    from ragbench.core.schema import ArtifactManifest, IndexedNode, RAGAnswer
...
class BasePipeline(ABC):
    ...
    def ingest(self, input_path: str, output_path: str) -> ArtifactManifest:
        ...
        :func:`ragbench.core.measure.build_ingest_manifest` to build it so
    ...
    def query(self, question: str, mode: str = "full_rag") -> RAGAnswer:
        ...
        was populated via :func:`ragbench.core.measure.build_query_metadata`.
        ...
        for name, parameter in signature.parameters.items():
            ...
            if not hasattr(self, name):
                raise RuntimeError(
                    ...
                    "to an attribute so it can be persisted in artifact v5"
                )
    ...
    def load_artifact(self, artifact_path: str):
        ...
        from ragbench.indexing.artifacts import load_artifact
        ...
        if mismatches:
            ...
            raise RuntimeError(f"Pipeline configuration does not match artifact v5: {details}")
...
def load_pipeline_class(technique_id: str) -> type[BasePipeline] | None:
    ...
    if technique_id not in _technique_ids():
        return None
    module = importlib.import_module(f"ragbench.techniques.{technique_id}.pipeline")
...
def list_pipelines(root: str | Path | None = None) -> list[dict[str, Any]]:
    ...
    package_root = resources.files("ragbench.techniques")
    for technique_id in _technique_ids():
        ...
        metadata["_package"] = f"ragbench.techniques.{technique_id}"
...
def _technique_ids() -> list[str]:
    root = resources.files("ragbench.techniques")
    return sorted(
        item.name
