...
class BaseParser(ABC):
    ...
    def parse(self, path: str, *, root: str | Path | None = None) -> list[DocumentBlock]:
        ...
        ``root`` is the ingest input path passed to the pipeline — when given,
        implementations should derive ``doc_id`` from the path relative to it
        (not the bare filename stem) so two identically-named files in
        different subdirectories don't collide. See
        ``ragbench.core.io.relative_doc_id``.
        """
