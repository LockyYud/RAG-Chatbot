...
# A dataset's role in *this* protocol: "dev" is where config, prompts, and
# chunk sizes get tuned; "test" is held out for empirical claims (see
# benchmarks.suites.claim_eligibility, which requires "test").
#
# Deliberately separate from ``metadata.split``, which records the upstream
# split the data came from and asserts nothing about held-out status — upstream
# "validation" is a dev split under another name. Nothing infers one from the
# other: only the person running the protocol knows whether a snapshot has been
# tuned against, so this is never derived and never defaulted. An absent value
# means nobody has asserted anything, which reads as "unsupported".
PROTOCOL_SPLITS = ("dev", "test")


def _inherited_protocol_split(source: Path) -> dict[str, str]:
    manifest_path = source / "manifest.json"
    if not manifest_path.exists():
        return {}
    metadata = read_json(manifest_path).get("metadata")
    protocol_split = metadata.get("protocol_split") if isinstance(metadata, dict) else None
    return {"protocol_split": protocol_split} if protocol_split in PROTOCOL_SPLITS else {}


def validate_protocol_split(protocol_split: str | None) -> str | None:
    if protocol_split is None:
        return None
    if protocol_split not in PROTOCOL_SPLITS:
        allowed = ", ".join(repr(value) for value in PROTOCOL_SPLITS)
        raise ValueError(f"protocol_split must be one of {allowed}; got {protocol_split!r}")
...
def sample_processed_dataset(
    ...
    dataset = PreparedDataset(
        ...
        metadata={
            ...
            "sample_limit": limit,
            "seed": seed,
            # A sample of a tuning set is still a tuning set. Dropping the
            # label would leave the sample "unspecified" — safe against
            # claims, but it also erases the provenance, so someone could
            # later label the sample "test" without seeing that its parent
            # was tuned against. Carried over rather than re-derived; only
            # present when the source declared it.
            **_inherited_protocol_split(source),
        },
    )
...
def _protocol_split_note(protocol_split: Any) -> str:
    if protocol_split == "dev":
        return "dev — tuning set; config may be chosen against it, so it cannot support an empirical claim"
    if protocol_split == "test":
        return "test — held out for claims; do not tune against it"
    return "unspecified — nobody has declared this snapshot held out, so it cannot support an empirical claim"
