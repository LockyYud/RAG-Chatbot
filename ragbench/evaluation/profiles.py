...
def validate_profile(profile: str, items: list[EvalItem], *, coverage: dict[str, Any] | None = None) -> None:
    ...
    total = len(items)
    retrieval_ratio = coverage.get("min_retrieval_coverage")
    # Every profile publishes retrieval metrics (recall/nDCG/MRR), so a
    # declared retrieval-coverage threshold is enforced for all of them —
    # otherwise a claim-eligible RAG suite could be made to declare the value
    # without it constraining anything. The *undeclared* fallback ("at least
    # one item with a qrel") stays scoped to the retrieval profile: a RAG
    # dataset with no qrels at all is a legitimate answer-quality-only
    # evaluation, and this must not start rejecting it.
    if profile == "retrieval" or retrieval_ratio is not None:
        covered = sum(1 for item in items if item.expected_doc_ids or item.expected_chunk_ids)
        _require_coverage(covered, total, retrieval_ratio, profile, "a qrel (expected_doc_ids/chunk_ids)")
