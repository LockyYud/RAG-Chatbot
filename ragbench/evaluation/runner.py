...
from ragbench.datasets.synthetic import load_synthetic_manifest, validate_synthetic_dataset
from ragbench.evaluation.audit import load_recorded_audit
...
def run_eval(
    ...
    if Path(dataset_path).is_dir():
        # A generated set has qa.jsonl and a manifest but no documents/queries/
        # qrels triple, so it gets its own validator rather than failing the
        # fixed-dataset one for the wrong reason.
        if load_synthetic_manifest(dataset_path) is not None:
    ...
    # A model-labelled dataset must never produce a bare results table: the
    # trust block travels with the numbers, and says up front how far they go.
    synthetic_manifest = load_synthetic_manifest(resolved_dataset_path)
    if synthetic_manifest is not None:
        from ragbench.evaluation.trust import build_trust_block

        trust_block = build_trust_block(
            synthetic_manifest,
            audit=load_recorded_audit(synthetic_manifest),
            judge_model=(judge_spec or {}).get("params", {}).get("model") if judge_spec else None,
        )
        if trust_block is not None:
            report["trust"] = trust_block
...
def _run_metadata(
    ...
    return {
        ...
        # Two different facts. `dataset_split` is the *upstream* split the data
        # came from ("validation", "test", ...) and asserts nothing about
        # held-out status — the Vietnamese retrieval dataset is upstream
        # "validation", i.e. a dev split under another name.
        # `dataset_protocol_split` is this protocol's own dev/test role for it,
        # which only a human can assert; claim-eligible runs require "test"
        # (see suites.claim_eligibility). Both default to "unspecified" rather
        # than None so a report reader can tell "not marked" from "marked".
        "dataset_split": dataset_manifest.get("metadata", {}).get("split", "unspecified"),
        "dataset_protocol_split": dataset_manifest.get("metadata", {}).get("protocol_split", "unspecified"),
