...
**Use chunk labels only when every technique being compared shares the chunker that produced them.** A `chunk_id` is
derived from the chunker's own windowing (`processing/chunkers/common.chunk_id`), so labels built with one chunker
never match a technique that chunks differently, and because chunk labels take precedence whenever present, the
mismatch scores that technique at 0 on every retrieval metric while `retrieval_evaluated` stays true. Fixed dataset
adapters and synthetic generation both therefore emit `expected_chunk_ids: []` and label at document level.
`metrics.retrieval_label_level` reports which identifier a run's qrels actually used.
