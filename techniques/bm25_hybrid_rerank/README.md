...
- `RRFHybridRetriever` (`raglab/indexing/retrievers.py`): runs `BM25Retriever`
  and `DenseRetriever`, fuses their lists via `reciprocal_rank_fusion(k=60)`.
- `CrossEncoderReranker` (`raglab/inference/rerankers/cross_encoder.py`): scores
...
```bash
raglab bench --techniques naive_rag parent_child bm25_hybrid_rerank \
